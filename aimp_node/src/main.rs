...
struct Args {
    ...
    /// Run without TUI dashboard (headless mode for containers/benchmarks)
    #[arg(long)]
    headless: bool,

    /// Comma-separated list of peer addresses (host:port) for explicit peering
...
struct NodeState {
    ...
    decision_engine: Arc<DecisionDispatcher>,
...
async fn main() -> aimp_node::AimpResult<()> {
    ...
    // 3. Local Deterministic Decision Engine (No external dependencies)
    let decision_engine = Arc::new(DecisionDispatcher::new(Some(log_tx.clone()))?);
    ...
    if args.headless {
        ...
        tokio::select! {
            _ = tokio::signal::ctrl_c() => {
                eprintln!("[AIMP] Received SIGINT, shutting down...");
            }
            _ = sigterm.recv() => {
                eprintln!("[AIMP] Received SIGTERM, shutting down...");
            }
        }
    ...
    if let Ok(res) = state.decision_engine.run_evaluation(&prompt, "{}").await {
        ...
        if let Ok(deterministic_json) = serde_json::to_string(&ordered_map) {
            ...
            if let Ok(sig_result) = state.identity.sign(data.clone()) {
                ...
                // Construct AiEvidence for auditable trail
                let evidence = crate::crdt::merkle_dag::AiEvidence {
                    prompt: prompt.clone(),
                    decision: deterministic_json.clone(),
                    model_hash: state.decision_engine.get_engine_hash(),
