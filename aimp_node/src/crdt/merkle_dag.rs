...
impl MerkleCrdtEngine {
    ...
    pub fn get_merkle_root(&mut self) -> Hash32 {
        ...
        let root = SecurityFirewall::hash(&combined);
        self.cached_root = Some(root);
        root
    }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_dag_convergence(
            ...
            let node1 = DagNode { parents: vec![], signature: sig1, data_hash: data1, vclock: vclocks[0].clone(), evidence: None };
            let node2 = DagNode { parents: vec![], signature: sig2, data_hash: data2, vclock: vclocks[1].clone(), evidence: None };
